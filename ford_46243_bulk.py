...
ITERATIONS = 10000  # REQUESTS (will affect duration)
CONCURRENCY = 20  # MAX CONCURRENCY (macs die past 30)
BULK_REQUESTS_COUNT = 1000  # DOCS TO INSERT PER REQUEST
...
def main():
    ...
    multiprocessing.Pool(CONCURRENCY).map(insert, range(ITERATIONS))
...
def insert(i):
    ...
    id_1 = id_factory(value=((i + 5000000) * BULK_REQUESTS_COUNT))
    id_2 = id_factory()

    for j in range(BULK_REQUESTS_COUNT):
        d_id = next(id_2)
        id_val = next(id_1)
        ...
        requests.append(pymongo.InsertOne(doc))

    resp = collection.bulk_write(requests)
    if not resp:
        print(
