...
class FormTemplate(object):
    ...
    Use <object>.add_title("Title") for title
    Use <object>.add_wellcome_message("Message", side=None) for top message
    Use str_param = form.add_text_field(name="name", default="default", info="info")
    Use int_param = form.add_int_field(name="name", default=1, info="info")
    Use bool_param = form.add_bool_field(name="name", default=True, info="info")
    """
    ...
    def set_action_button(self, name="Run", action=None):
        ...
        text = "Use <object>.set_action_button(name, lambda: func(args))\n" \
               "Use <object>.add_title(\"Title\") for title\n" \
               "Use <object>.add_wellcome_message(\"Message\", side=None) for top message\n" \
               "Use str_param = form.add_text_field(name=\"name\", default=\"default\", info=\"info\")\n" \
               "Use int_param = form.add_int_field(name=\"name\", default=1, info=\"info\")\n" \
...
class TestForm(object):
    ...
    def test_blank_form(self):
        form = FormTemplate(title="BLANK FORM")
        form.open()

    def test_filled_form(self):
        form2 = FormTemplate()

        form2.add_title("MODIFIED FORM")
